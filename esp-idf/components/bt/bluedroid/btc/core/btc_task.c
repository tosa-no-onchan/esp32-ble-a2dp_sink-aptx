...
#define DUBUG_NISHI11
#ifdef DUBUG_NISHI11
static char *btc_pid_t_nishi[]={
    "BTC_PID_MAIN_INIT",	// = 0,
    "BTC_PID_DEV",
    "BTC_PID_GATTS",
#if (GATTC_INCLUDED == TRUE)
...
static char* get_btc_pid_t_nishi(UINT8 i){
	return btc_pid_t_nishi[i];
}
#endif
    ...
    for (;;) {
        if (pdTRUE == xQueueReceive(xBtcQueue, &msg, (portTickType)portMAX_DELAY)) {
            ...
            case BTC_SIG_API_CALL:
            	// debug by nishi
                BTC_TRACE_DEBUG("%s(): msg sig=%u:BTC_SIG_API_CALL, pid=%u:%s ,act=%u arg=%p", __func__, msg.sig, msg.pid,get_btc_pid_t_nishi(msg.pid), msg.act, msg.arg);
                profile_tab[msg.pid].btc_call(&msg);
                break;
            case BTC_SIG_API_CB:
            	// debug by nishi
                BTC_TRACE_DEBUG("%s(): msg sig=%u:BTC_SIG_API_CB, pid=%u:%s ,act=%u arg=%p", __func__, msg.sig, msg.pid,get_btc_pid_t_nishi(msg.pid), msg.act, msg.arg);
                profile_tab[msg.pid].btc_cb(&msg);
                break;
            default:
            	// debug by nishi
                BTC_TRACE_DEBUG("%s(): msg sig=%u, pid=%u ,act=%u arg=%p", __func__, msg.sig, msg.pid, msg.act, msg.arg);
                break;
            }
            if (msg.arg) {
                osi_free(msg.arg);
...
bt_status_t btc_transfer_context(btc_msg_t *msg, void *arg, int arg_len, btc_arg_deep_copy_t copy_func)
{
    ...
    // changed by nishi
    BTC_TRACE_DEBUG("%s(): msg msg->sig=%u, msg->pid=%u, msg->act=%u arg=%p , caller=%p", __func__, msg->sig, msg->pid, msg->act,arg,__builtin_return_address(0));
