...
// changed by nishi 2019.1.31
//#define MAX_OUTPUT_A2DP_SNK_FRAME_QUEUE_SZ     (28)
...
#endif
// add by nishi for APTX 2019.2.15
...
static void btc_a2dp_sink_task_handler(void *arg)
{
    ...
    for (;;) {
		...
		#else
        if (xActivatedMember == btc_aa_snk_ctrl_queue) {
		#endif
...
bool btc_a2dp_sink_startup(void)
{
    ...
    APPL_TRACE_EVENT("%s(): ## A2DP SINK START MEDIA THREAD ##",__func__);
    ...
    // changed by nishi for APTX 2019.2.15
    ...
    // DEBUG by nishi 2019.2.2
    APPL_TRACE_WARNING("%s(): BTC_A2DP_SINK_TASK_PINNED_TO_CORE=%d",__func__,BTC_A2DP_SINK_TASK_PINNED_TO_CORE);
    ...
    // add by nishi for APTX
    //btc_a2dp_sink_data_ready_task
    ...
    // add by nishi for disable watch dog 2019.2.17
    //esp_task_wdt_delete(btc_aa_snk_task_dr_hdl);

    btc_a2dp_sink_ctrl_post(BTC_MEDIA_TASK_SINK_INIT, NULL);

    APPL_TRACE_EVENT("%s(): ## A2DP SINK MEDIA THREAD STARTED ##",__func__);
...
error_exit:;
    ...
    // add by nishi for APTX 2019.2.15
    if (btc_aa_snk_task_dr_hdl != NULL) {
        vTaskDelete(btc_aa_snk_task_dr_hdl);
        btc_aa_snk_task_dr_hdl = NULL;
    }
...
{
    APPL_TRACE_EVENT("%s(): ## A2DP SINK STOP MEDIA THREAD ##",__func__);
...
{
    APPL_TRACE_EVENT("%s(): btc reset decoder",__func__);
...
/*
 * void btc_a2dp_sink_data_ready_task(void *arg)
 * こちらは、実験版です。
 *
 */
static void btc_a2dp_sink_data_ready_task(void *arg)
{
	...
	//#define BTC_A2DP_SINK_DATA_TIMER (100UL)		// この半分でも良いみたい
	//#define BTC_A2DP_SINK_DATA_TIMER (50UL)		// 少し多いか?
    ...
    //gpio_set_level(GPIO_NUM_17, 0);  // Low
    ...
    for (;;) {
		...
		// DEBUG by nishi
		APPL_TRACE_DEBUG("%s():#2 passed!",__func__);
		...
		if(xStatus == pdPASS)
    	{
            ...
            if (data_evt == BTC_A2DP_SINK_DATA_EVT)
            {
            	while(1){
                    ...
                    // DEBUG by nishi
                   	APPL_TRACE_DEBUG("%s():#3 passed!",__func__);
    				...
    				//gpio_set_level(GPIO_NUM_17, 1);
       				if (btc_a2dp_sink_state == BTC_A2DP_SINK_STATE_ON){
       					...
       					// DEBUG by nishi
       					APPL_TRACE_DEBUG("%s():#6 btc_aa_snk_cb.codec_type=%x",__func__,btc_aa_snk_cb.codec_type);
					...
					//#define TEST_NISHI_BTC_1
					#ifdef TEST_NISHI_BTC_1
    				//i++;
    				if(xLastWakeTime0 < xTaskGetTickCount()){
    				//if(i > 250){
    					// Watch Dog 対応  by nishi 2019.2.15
    					//vTaskDelay(1);
    					esp_task_wdt_delete(btc_aa_snk_task_dr_hdl);
    					//esp_task_wdt_feed(btc_aa_snk_task_dr_hdl);
    					//i=0;
    					xLastWakeTime0 = xTaskGetTickCount() + BTC_A2DP_SINK_DATA_TIMER;
    				}
					#endif
...
static void btc_a2dp_sink_handle_decoder_reset_Sbc(tBTC_MEDIA_SINK_CFG_UPDATE *p_msg)
{
    ...
    // set sampleRate
    btc_aa_snk_cb.sample_rate = btc_a2dp_sink_get_track_frequency(p_buf->codec_info[2],sbc_cie.samp_freq);
    // set channelMode  stero/mono/mix
    btc_aa_snk_cb.channel_count = btc_a2dp_sink_get_track_channel_count(p_buf->codec_info[2],sbc_cie.ch_mode);
    // set codec_type
    btc_aa_snk_cb.codec_type = p_buf->codec_info[2];  // add by nishi for support APTX 2019.2.25
    ...
    case A2D_SBC_IE_SAMP_FREQ_16:
    	APPL_TRACE_EVENT("\tsamp_freq:%d (16000)", sbc_cie.samp_freq);
        freq_multiple = 16 * 20;
        break;
    case A2D_SBC_IE_SAMP_FREQ_32:
    	APPL_TRACE_EVENT("\tsamp_freq:%d (32000)", sbc_cie.samp_freq);
        freq_multiple = 32 * 20;
        break;
    case A2D_SBC_IE_SAMP_FREQ_44:
    	APPL_TRACE_EVENT("\tsamp_freq:%d (44100)", sbc_cie.samp_freq);
        freq_multiple = 441 * 2;
        break;
    case A2D_SBC_IE_SAMP_FREQ_48:
    	APPL_TRACE_EVENT("\tsamp_freq:%d (48000)", sbc_cie.samp_freq);
        freq_multiple = 48 * 20;
        break;
    default:
    ...
    case A2D_SBC_IE_CH_MD_MONO:
    	APPL_TRACE_EVENT("\tch_mode:%d (Mono)", sbc_cie.ch_mode);
        break;
    case A2D_SBC_IE_CH_MD_DUAL:
    	APPL_TRACE_EVENT("\tch_mode:%d (DUAL)", sbc_cie.ch_mode);
        break;
    case A2D_SBC_IE_CH_MD_STEREO:
    	APPL_TRACE_EVENT("\tch_mode:%d (STEREO)", sbc_cie.ch_mode);
        break;
    case A2D_SBC_IE_CH_MD_JOINT:
    	APPL_TRACE_EVENT("\tch_mode:%d (JOINT)", sbc_cie.ch_mode);
        break;
    default:
    ...
    case A2D_SBC_IE_BLOCKS_4:
    	APPL_TRACE_EVENT("\tblock_len:%d (4)", sbc_cie.block_len);
        num_blocks = 4;
        break;
    case A2D_SBC_IE_BLOCKS_8:
    	APPL_TRACE_EVENT("\tblock_len:%d (8)", sbc_cie.block_len);
        num_blocks = 8;
        break;
    case A2D_SBC_IE_BLOCKS_12:
    	APPL_TRACE_EVENT("\tblock_len:%d (12)", sbc_cie.block_len);
        num_blocks = 12;
        break;
    case A2D_SBC_IE_BLOCKS_16:
    	APPL_TRACE_EVENT("\tblock_len:%d (16)", sbc_cie.block_len);
        num_blocks = 16;
        break;
    default:
    	APPL_TRACE_EVENT("\tUnknown BlockLen ");
        break;
    }
    ...
    APPL_TRACE_EVENT("\tBit pool Min:%d Max:%d", sbc_cie.min_bitpool, sbc_cie.max_bitpool);

    int frames_to_process = ((freq_multiple) / (num_blocks * num_subbands)) + 1;
    APPL_TRACE_EVENT(" Frames to be processed in 20 ms %d", frames_to_process);
}
...
static void btc_a2dp_sink_handle_decoder_reset_Aptx(tBTC_MEDIA_SINK_CFG_UPDATE *p_msg)
{
    ...
    // set sampleRate
    btc_aa_snk_cb.sample_rate = btc_a2dp_sink_get_track_frequency(p_buf->codec_info[2],aptx_cie.sampleRate);
    // set channelMode  stero/mono
    btc_aa_snk_cb.channel_count = btc_a2dp_sink_get_track_channel_count(p_buf->codec_info[2],aptx_cie.channelMode);
    // set codec_type
    btc_aa_snk_cb.codec_type = p_buf->codec_info[2];
...
static void btc_a2dp_sink_handle_inc_media(tBT_SBC_HDR *p_msg)
{
    ...
    APPL_TRACE_EVENT("Number of sbc frames %d, frame_len %d", num_sbc_frames, sbc_frame_len);
    APPL_TRACE_EVENT("\tp_msg->len=x%x,p_msg->offset=x%x", p_msg->len,p_msg->offset);

   	for (count = 0; count < num_sbc_frames && sbc_frame_len != 0; count ++) {
   		pcmBytes = availPcmBytes;
		status = OI_CODEC_SBC_DecodeFrame(
				&btc_sbc_decoder_context,		//OI_CODEC_SBC_DECODER_CONTEXT *context
				(const OI_BYTE **)&sbc_start_frame, // const OI_BYTE **frameData
               	(OI_UINT32 *)&sbc_frame_len,	// OI_UINT32 *frameBytes  入力フレームバイト数
				(OI_INT16 *)pcmDataPointer,		// OI_INT16 *pcmData
...
static void btc_a2dp_sink_handle_inc_media_Aptx(tBT_SBC_HDR *p_msg)
{
    ...
    //UINT8 *sbc_start_frame = ((UINT8 *)(p_msg + 1) + p_msg->offset + 1);
    UINT8 *sbc_start_frame = ((UINT8 *)(p_msg + 1) + p_msg->offset);  // tBT_SBC_HDR を飛ばした位置 + p_msg->offset
	...
	//#define APTX_CTX_RESET
	#ifdef APTX_CTX_RESET
    // 毎回、リセットしない方が良い。
    // I (156692) BT_AV: A2DP audio state: Started の時、初期化かリセット、すればよいか。
    if(ctx!=NULL){
    	aptx_reset(ctx);
    }
	#endif

    // ここは、aptx_decode_16bit() の値に合わせます。
    sample_size = aptx_get_sample_size(ctx);	// 一度に処理する、サンプルサイズみたい
    ...
    offset=0;
   	...
   	for (count = 0; sbc_frame_len >= sample_size; count ++) {
		...
		processed = aptx_decode_16bit(
				...
				(unsigned char *)(sbc_start_frame + offset),	// const unsigned char *input 入力バッファポインタ
				(size_t)sbc_frame_len,				// size_t input_size  入力データ長
				(int16_t *)pcmDataPointer,	// unsigned char *output  出力バッファ
				(size_t)availPcmBytes,				// size_t output_size  出力バッファ長
				(size_t *)&pcmBytes);				// size_t *written   出力されたデータ長
		...
		offset += processed;
   		...
   		p_msg->offset = offset;				// BT_SBC_HDR->offset オフセットの更新
   		p_msg->len = sbc_frame_len;			// BT_SBC_HDR->len データ長の更新

		// 一度ここで breakさせますよ。
		//break;

		availPcmBytes -= pcmBytes;			// 残り出力バッファサイズ　の更新
   		pcmDataPointer += pcmBytes / 2;		// 出力バッファポインタの更新
    ...
    //btc_a2d_data_cb_to_app((uint8_t *)btc_sbc_pcm_data, (BTC_SBC_DEC_PCM_DATA_LEN * sizeof(OI_INT16) - availPcmBytes));
   	...
   	len=BTC_SBC_DEC_PCM_DATA_LEN * sizeof(OI_INT16) - availPcmBytes;
    //len=pcmDataPointer-btc_sbc_pcm_data;
    btc_a2d_data_cb_to_app((uint8_t *)btc_sbc_pcm_data, len);
...
UINT8 btc_a2dp_sink_enque_buf(BT_HDR *p_pkt)
{
    ...
    // DEBUG by nishi
    //APPL_TRACE_EVENT("%s():#2 p_pkt->len=%d, p_pkt->offset=%d",__FUNCTION__,p_pkt->len,p_pkt->offset);

    /* allocate and Queue this buffer */
    if ((p_msg = (tBT_SBC_HDR *) osi_malloc(sizeof(tBT_SBC_HDR) +
                                            p_pkt->offset + p_pkt->len)) != NULL) {
        memcpy(p_msg, p_pkt, (sizeof(BT_HDR) + p_pkt->offset + p_pkt->len));
    	// test by nishi 2019.2.2
        //memmove(p_msg, p_pkt, (sizeof(BT_HDR) + p_pkt->offset + p_pkt->len));
        ...
        // btc_a2dp_sink_data_que へ タスク キューイング
        e_id = BTC_A2DP_SINK_DATA_EVT;
