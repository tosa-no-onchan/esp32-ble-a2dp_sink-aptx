...
//add by nishi for eclipse edit of a2dp_sink test
...
static void bte_av_media_callback(tBTA_AV_EVT event, tBTA_AV_MEDIA *p_data)
{
    ...
    if (event == BTA_AV_MEDIA_DATA_EVT) { /* Switch to BTC_MEDIA context */
    	// add by nishi
        //APPL_TRACE_WARNING("bte_av_media_callback() : #2 passed");
        ...
        if ( (state == BTC_AV_STATE_STARTED) || /* send SBC packets only in Started State */
            ...
            BTC_TRACE_DEBUG("%s():#2 Packets in Que %d\n", __func__, que_len);
        }
        else {
        	// バッファの開放を、bta_av_media_callback() から、下位で行わせる。 by nishi 2019.2.2
			//#define TEST_NISHI_BTC_AV1
			#ifdef TEST_NISHI_BTC_AV1
        	osi_free(p_data);
			#endif
