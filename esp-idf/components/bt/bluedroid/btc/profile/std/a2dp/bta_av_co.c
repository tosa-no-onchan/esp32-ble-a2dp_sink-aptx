    ...
    *p_protect_info = 0;
    ...
    if (tsep == AVDT_TSEP_SRC) {
        ...
        *p_codec_type = BTA_AV_CODEC_SBC;
		...
		#define NISHI_APTX_1
		#ifdef NISHI_APTX_1
		    // ここで、APTX  コーデックタイプを設定しています。 by nishi 2019.1.15
    		*p_codec_type = BTA_AV_CODEC_VEND;
    		...
    		// Windows10 + ドングルでの、bta_av_co_audio_getconfig() コールテスト用に、
    		// *p_codec_type = でたらめな値をセット。
    		//*p_codec_type = 0xfb;
		...
		#else
    		// ここで、SBC コーデックタイプを設定しています。 by nishi 2019.1.15
    		*p_codec_type = BTA_AV_CODEC_SBC;
                                  ...
                                  UINT8 *p_protect_info)
{
    ...
    if (supported) {
        ...
        if (p_peer->num_sup_srcs < BTA_AV_CO_NUM_ELEMENTS(p_peer->srcs)) {
            ...
            APPL_TRACE_ERROR("bta_av_audio_sink_getconfig() #6 no more room for SRC info");
        }
    }
    // add by nishi
    else{
    ...
    /* supported codec caps で、問い合わせる件数を、SRC が前もって、知らせてきているのか?  */
    if ((p_peer->num_rx_srcs == p_peer->num_srcs) ||
            (p_peer->num_sup_srcs == BTA_AV_CO_NUM_ELEMENTS(p_peer->srcs))) {
        APPL_TRACE_DEBUG("bta_av_audio_sink_getconfig() #8 last SRC reached");
        ...
        if (bta_av_co_audio_peer_src_supports_codec(p_peer, &index)) {
            ...
            /* Build the codec configuration for this sink */
            {
                ...
                APPL_TRACE_DEBUG("bta_av_audio_sink_getconfig() #10  p_codec_info[%x:%x:%x:%x:%x:%x]",
...

{
    ...
    if (p_peer->uuid_to_connect == UUID_SERVCLASS_AUDIO_SOURCE) {
    	// a2dp_sink こちらへ行くみたい。 by nishi 2019.1.19
    ...
    APPL_TRACE_DEBUG("%s(): #5 handle:0x%x codec_type:%d seid:%d",__func__,hndl, codec_type, seid);
    APPL_TRACE_DEBUG("num_protect:0x%02x protect_info:0x%02x%02x%02x",
                     *p_num_protect, p_protect_info[0], p_protect_info[1], p_protect_info[2]);

    APPL_TRACE_DEBUG("%s(): #6 peer(o=%d,n_snks=%d,n_rx_snks=%d,n_sup_snks=%d)",__func__,
    ...
    if (supported) {
        ...
        if (p_peer->num_sup_snks < BTA_AV_CO_NUM_ELEMENTS(p_peer->snks)) {
            ...
            APPL_TRACE_DEBUG("%s(): #7 saved caps[%x:%x:%x:%x:%x:%x]",__func__,
                             p_codec_info[1], p_codec_info[2], p_codec_info[3],
                             p_codec_info[4], p_codec_info[5], p_codec_info[6]);
            ...
            APPL_TRACE_ERROR("%s(): #8 no more room for SNK info",__func__);
        }
    }
    ...
    if ((p_peer->num_rx_snks == p_peer->num_snks) ||
        ...
        APPL_TRACE_DEBUG("%s(): #9 last sink reached",__func__);
        ...
        if (bta_av_co_audio_peer_supports_codec(p_peer, &index)) {
            ...
            if (bta_av_co_audio_codec_build_config(p_sink->codec_caps, codec_cfg)) {
                APPL_TRACE_DEBUG("%s(): #10 reconfig p_codec_info[%x:%x:%x:%x:%x:%x]",__func__,
                ...
                *p_num_protect = 0;
                ...
                if (p_peer->acp) {
                    if (p_peer->recfg_needed) {
                        APPL_TRACE_DEBUG("%s(): #11 call BTA_AvReconfig(x%x)", __func__,hndl);
                               ...
                               UINT8 t_local_sep, UINT8 avdt_handle)
{
    ...
    // test by nishi
    // ここは、Winddows10 で、bta_av_co_audio_getconfig() を来させるためのエラー設定です。
    //p_codec_info[0]=7;
    ...
    p_peer = bta_av_co_get_peer(hndl);
    if (p_peer == NULL) {
        APPL_TRACE_ERROR("%s(): #5 could not find peer entry",__func__);
    ...
    if (p_peer->opened) {
        APPL_TRACE_ERROR("%s(): #7 peer already in use",__func__);
    }
    ...
    if (num_protect != 0) {
        ...
        if ((num_protect != 1) ||
            ...
            APPL_TRACE_ERROR("%s(): #8 wrong CP configuration",__func__);
    ...
    if (num_protect != 0) {
        APPL_TRACE_ERROR("%s(): #9 wrong CP configuration",__func__);
        status = A2D_BAD_CP_TYPE;
        category = AVDT_ASC_PROTECT;
    }
#endif
    if (status == A2D_SUCCESS) {
        ...
        if (codec_cfg_supported) {
            ...
            case BTC_AV_CODEC_SBC:
                ...
                APPL_TRACE_EVENT("#12 remote peer setconfig bitpool range [%d:%d]",
            ...
            default:
            	APPL_TRACE_ERROR("%s(): #13 unsupported cid %d", __func__,bta_av_co_cb.codec_cfg.id);
                recfg_needed = TRUE;
                break;
            }
            /* Protect access to bta_av_co_cb.codec_cfg */
    ...
    if (status != A2D_SUCCESS) {
    	APPL_TRACE_DEBUG("%s(): #14 reject status=%d category=%d", __func__,status, category);
                          ...
                          UINT16 mtu)
{
    ...
    APPL_TRACE_DEBUG("%s(): #2 mtu:%d codec_type:%d", __func__,mtu, codec_type);
                                    ...
                                    UINT32 *p_timestamp)
{
    ...
    if (p_buf != NULL) {
        ...
        default:
            APPL_TRACE_ERROR("%s():#2 Unsupported codec type (%d)", __func__,codec_type);
            break;
        }
#if defined(BTA_AV_CO_CP_SCMS_T) && (BTA_AV_CO_CP_SCMS_T == TRUE)
        {
...
void bta_av_co_audio_drop(tBTA_AV_HNDL hndl)
{
    ...
    APPL_TRACE_ERROR("%s(): x%x", __func__,hndl);
}
...
static BOOLEAN bta_av_co_audio_codec_build_config(const UINT8 *p_codec_caps, UINT8 *p_codec_cfg)
{
    ...
    case BTC_AV_CODEC_SBC:
        ...
        APPL_TRACE_EVENT("%s(): #2 bitpool min %d, max %d",__func__,
                         p_codec_cfg[BTA_AV_CO_SBC_MIN_BITPOOL_OFF],
                         p_codec_caps[BTA_AV_CO_SBC_MAX_BITPOOL_OFF]);
        break;
    default:
        APPL_TRACE_ERROR("%s(): #3 unsupported codec id %d", __func__,bta_av_co_cb.codec_cfg.id);
        return FALSE;
        break;
    }
    return TRUE;
...
static BOOLEAN bta_av_co_audio_codec_cfg_matches_caps(UINT8 codec_id, const UINT8 *p_codec_caps, const UINT8 *p_codec_cfg)
{
    ...
    case BTC_AV_CODEC_SBC:
        ...
        APPL_TRACE_EVENT("%s(): #2 min %d/%d max %d/%d",__func__,
    ...
    default:
        APPL_TRACE_ERROR("%s(): #3 unsupported codec id %d", __func__,codec_id);
        return FALSE;
        break;
    }
...
static BOOLEAN bta_av_co_audio_peer_supports_codec(tBTA_AV_CO_PEER *p_peer, UINT8 *p_snk_index)
{
    ...
    for (index = 0; index < p_peer->num_sup_snks; index++) {
        if (p_peer->snks[index].codec_type == codec_type) {
            ...
            default:
                APPL_TRACE_ERROR("%s(): #2 unsupported codec id %d", __func__,bta_av_co_cb.codec_cfg.id);
                return FALSE;
                break;
            }
        }
...
static BOOLEAN bta_av_co_audio_peer_src_supports_codec(tBTA_AV_CO_PEER *p_peer, UINT8 *p_src_index)
{
    ...
    for (index = 0; index < p_peer->num_sup_srcs; index++) {
        if (p_peer->srcs[index].codec_type == codec_type) {
            ...
            default:
                APPL_TRACE_ERROR("%s(): #2 unsupported codec id %d",__func__,
        ...
        if (p_peer->opened) {
            if (bta_av_co_audio_peer_supports_codec(p_peer, &snk_index)) {
                ...
                if (!bta_av_co_audio_sink_supports_cp(p_sink)) {
                    APPL_TRACE_DEBUG("%s(): #2 sink %d of peer %d doesn't support cp",__func__,
                                     snk_index, index);
...
#endif
                        APPL_TRACE_DEBUG("%s():#3 call BTA_AvReconfig(x%x)", __func__,BTA_AV_CO_AUDIO_INDX_TO_HNDL(index));
                ...
                APPL_TRACE_DEBUG("%s():#4 index %d doesn't support codec", __func__,index);
                return FALSE;
            }
        }
    }
...
BOOLEAN bta_av_co_audio_set_codec(const tBTC_AV_MEDIA_FEEDINGS *p_feeding, tBTC_AV_STATUS *p_status)
{
    ...
    *p_status = BTC_ERROR_SRV_AV_FEEDING_NOT_SUPPORTED;
    ...
    APPL_TRACE_DEBUG("%s(): #2 cid=%d", __func__,p_feeding->format);
    ...
    case BTC_AV_CODEC_PCM:
        ...
        if ((p_feeding->cfg.pcm.num_channel != 1) &&
            ...
            APPL_TRACE_ERROR("%s(): #3 PCM channel number unsupported",__func__);
            return FALSE;
        }
        if ((p_feeding->cfg.pcm.bit_per_sample != 8) &&
                (p_feeding->cfg.pcm.bit_per_sample != 16)) {
            APPL_TRACE_ERROR("%s(): #4 PCM sample size unsupported",__func__);
            return FALSE;
        }
        ...
        case 44100:
            ...
            break;
        default:
            APPL_TRACE_ERROR("%s(): #5 PCM sampling frequency unsupported",__func__);
            return FALSE;
            break;
        }
        /* Build the codec config */
        if (A2D_BldSbcInfo(A2D_MEDIA_TYPE_AUDIO, &sbc_config, new_cfg.info) != A2D_SUCCESS) {
            APPL_TRACE_ERROR("%s(): #6 A2D_BldSbcInfo failed",__func__);
            return FALSE;
        }
        break;
    ...
    default:
        APPL_TRACE_ERROR("%s(): #7 Feeding format unsupported",__func__);
        return FALSE;
        break;
    }
...
void bta_av_co_audio_discard_config(tBTA_AV_HNDL hndl)
{
    ...
    p_peer = bta_av_co_get_peer(hndl);
    if (p_peer == NULL) {
        APPL_TRACE_ERROR("%s(): #1 could not find peer entry",__func__);
        return;
    }
...
BOOLEAN bta_av_co_peer_cp_supported(tBTA_AV_HNDL hndl)
{
    ...
    p_peer = bta_av_co_get_peer(hndl);
    if (p_peer == NULL) {
        APPL_TRACE_ERROR("%s(): #2 could not find peer entry",__func__);
        return FALSE;
    }
    ...
    APPL_TRACE_ERROR("%s(): #3 did not find SBC sink",__func__);
    return FALSE;
}
