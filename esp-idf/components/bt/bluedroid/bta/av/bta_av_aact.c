...
void bta_av_stream_data_cback(UINT8 handle, BT_HDR *p_pkt, UINT32 time_stamp, UINT8 m_pt)
{
	...
	// バッファの開放を、bta_av_media_callback() から、下位で行わせる。 by nishi 2019.2.2
	//#define TEST_NISHI_BTAAV_AACT1
	#ifndef TEST_NISHI_BTAAV_AACT1
...
void bta_av_config_ind (tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    // add by nishi
    //else if(p_evt_cfg->codec_info[2]==0x00){
    //    setconfig.hndl      = p_scb->hndl; /* we may not need this */
    //    setconfig.err_code  = AVDT_ERR_UNSUP_CFG;
    //    bta_av_ssm_execute(p_scb, BTA_AV_CI_SETCONFIG_FAIL_EVT, (tBTA_AV_DATA *) &setconfig);
    //}
...
void bta_av_setconfig_rsp (tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    APPL_TRACE_DEBUG("%s():#1 sep_idx: %d cur_psc_mask:0x%x",__func__, p_scb->sep_idx, p_scb->cur_psc_mask);
    ...
    if (p_data->ci_setconfig.err_code == AVDT_SUCCESS) {
        ...
        APPL_TRACE_DEBUG("%s():#2 recfg_needed:%d role:x%x num:%d",__func__,
...
void bta_av_str_opened (tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    p_scb->l2c_bufs = 0;
    // bta_av_a2d_cos::bta_av_co.s で定義されている各処理ルーチンをコール。 by nishi 2019.1.23
    // p_scb->p_cos->open()  -> bta_av_co_audio_open()
    p_scb->p_cos->open(p_scb->hndl,p_scb->codec_type, p_scb->cfg.codec_info, mtu);
...
void bta_av_getcap_results (tBTA_AV_SCB *p_scb, tBTA_AV_DATA *p_data)
{
    ...
    // APTX -> SBC へ、コーディック情報を戻す。 by nishi 2019.1.21
    if(p_scb->seps[0].codec_type!=BTA_AV_CODEC_SBC){

    	// avdt_cb.scb[n] 取得できるか?
    ...
    if ((p_scb->p_cap->num_codec != 0) &&
			...
			// bta_av_co_audio_getconfig() をコール by nishi 2019.1.20
