...
static tBTA_AV_SCB *bta_av_alloc_scb(tBTA_AV_CHNL chnl)
{
    ...
    // p_bta_av_cfg は、&bta_av_cfg で、スタティックな定義済
...
void bta_av_conn_cback(UINT8 handle, BD_ADDR bd_addr, UINT8 event, tAVDT_CTRL *p_data)
{
...
#endif
    {
        ...
        if (AVDT_DISCONNECT_IND_EVT == event) {
            ...
            APPL_TRACE_DEBUG("%s():#1 CONN_IND is ACP:%d\n",__func__, p_data->hdr.err_param);
        }
#endif
...
#if (defined(BTA_AV_DEBUG) && BTA_AV_DEBUG == TRUE)
            if (p_scb) {
                APPL_TRACE_DEBUG("%s():#2 scb hndl x%x, role x%x\n",__func__, p_scb->hndl, p_scb->role);
            }
#endif
            APPL_TRACE_DEBUG("%s():#3 conn_cback bd_addr:%02x-%02x-%02x-%02x-%02x-%02x\n",__func__,
                             bd_addr[0], bd_addr[1],
                             bd_addr[2], bd_addr[3],
                             bd_addr[4], bd_addr[5]);
...
static void bta_av_api_register(tBTA_AV_DATA *p_data)
{
    ...
    // DEBUG by nishi
    APPL_TRACE_DEBUG("%s()::bta_av_main.c #1 called!",__func__);
    ...
    do {
        ...
        // DEBUG by nishi
    	// registr.chnl -> 0x40 = BTA_AV_CHNL_AUDIO
        APPL_TRACE_DEBUG("%s():#2 registr.chnl=%x",__func__,registr.chnl);
        ...
        */
        ...
        APPL_TRACE_DEBUG("%s(): #3 nsc_mask: 0x%x",__func__, cs.nsc_mask);
        ...
        if (registr.chnl == BTA_AV_CHNL_AUDIO) {
            ...
            p_scb->p_cos     = registr.p_bta_av_cos;		// 処理ルーチンテーブルか? by nishi
            ...
            // DEBUG by nishi
            APPL_TRACE_DEBUG("%s(): #4 call (p_scb->p_cos->init)",__func__);
            // (p_scb->p_cos->init) は、 bta_av_co_audio_init()::bta_av_co.c です。
            while (index < BTA_AV_MAX_SEPS &&
                ...
                APPL_TRACE_DEBUG("%s(): #4.1 SEP Type = %d",__func__,cs.tsep);
#endif
                // memo by nishi AVDT_CreateStream()
                // avdt_cb.scb[n]の空いているエントリーをアサインして、cs の中身をコピーして、
                // &p_scb->seps[index].av_handle に handle を返す。
                if (AVDT_CreateStream(&p_scb->seps[index].av_handle, &cs) == AVDT_SUCCESS) {
                    ...
                    APPL_TRACE_DEBUG("%s(): #5 audio[%d] av_handle: %d codec_type: %d",__func__,
